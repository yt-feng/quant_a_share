...
- `Eastmoney public endpoint`：Vercel `/api/market` 当前用于全 A 股票池、全市场主力资金字段、指数、行业/概念板块、板块成分股、单股 K 线、涨停/炸板/强势股池、ETF 资金榜、东财人气榜、个股资金流、北向资金、沪深港通 KAMT 成交/累计流入、北向持股前排、公告等；默认向前端返回最多 6000 只股票，并暴露 `stockUniverse` 覆盖元信息。
...
- `BaoStock`：通过 GitHub Actions 批量生成 `pages/data/baostock-cache.json`，当前会从市场快照、财务缓存、热榜、资金和行业龙头扩展到最多 180 只股票，缓存历史 K 线、换手率、PE/PB/PS/PCF、MA 和区间收益。
